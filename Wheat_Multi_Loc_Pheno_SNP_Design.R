...
cor(ddd)[1,]
Name = c('TKWT', 'TSTWT', 'SPSM', 'KPS', 'KPSM', 'HT', 'I(TKWT^2)', 'I(TKWT*TSTWT)', 'I(TKWT*SPSM)',
         'I(TKWT*KPS)', 'I(TKWT*KPSM)', 'I(TKWT*HT)', 'I(TSTWT^2)', 'I(TSTWT*SPSM)',
         'I(TSTWT*KPS)', 'I(TSTWT*KPSM)', 'I(TSTWT*HT)', 'I(SPSM^2)', 'I(SPSM*KPS)', 'I(SPSM*KPSM)',
         'I(SPSM*HT)', 'I(KPS^2)', 'I(KPS*KPSM)', 'I(KPS*HT)', 'I(KPSM^2)', 'I(KPSM*HT)', 'I(HT^2)',
...
######################################################################################################
# Order of inclusion of covariates using SNP data in multi-location analysis
######################################################################################################
# [1] "I(TKWT*KPSM)"  "I(TSTWT*KPS)"  "KPSM"          "I(SPSM^2)"     "I(KPS*HT)"    
#[6] "I(KPSM^2)"     "I(TKWT*SPSM)"  "I(TKWT*HT)"    "TSTWT"         "I(SPSM*KPS)"  
#[11] "I(KPS^2)"      "I(TSTWT^2)"    "SPSM"          "I(SPSM*KPSM)"  "I(TSTWT*HT)"  
#[16] "I(TKWT*TSTWT)" "TKWT"          "I(TSTWT*KPSM)" "I(TKWT^2)"     "I(TKWT*KPS)"  
#[21] "I(TSTWT*SPSM)" "I(TKWT*SPSM)"  "I(HT^2)"       "I(KPS*KPSM)"   "HT"           
#[26] "barc67"        "I(KPSM*HT)"    "cmwg680bcd366" "bcd141"        "barc86"       
#[31] "gwm155"        "KPS"           "I(SPSM*HT)" 

AA = c( "I(TKWT*KPSM)",  "I(TSTWT*KPS)",  "KPSM", "I(SPSM^2)",  "I(KPS*HT)",    
        "I(KPSM^2)", "I(TKWT*SPSM)","I(TKWT*HT)" , "TSTWT" ,"I(SPSM*KPS)" , 
        "I(KPS^2)", "I(TSTWT^2)",  "SPSM",  "I(SPSM*KPSM)","I(TSTWT*HT)",  
        "I(TKWT*TSTWT)", "TKWT" ,  "I(TSTWT*KPSM)", "I(TKWT^2)", "I(TKWT*KPS)" , 
        "I(TSTWT*SPSM)", "I(TKWT*SPSM)",  "I(HT^2)" ,      "I(KPS*KPSM)",   "HT",           
        "barc67","I(KPSM*HT)" , "cmwg680bcd366", "bcd141","barc86",     
        "gwm155", "KPS","I(SPSM*HT)" )
Big_order = lm(YIELD ~ I(TKWT*KPSM) + I(TSTWT*KPS) + KPSM + I(SPSM^2) + I(KPS*HT) +   
                 I(KPSM^2) + I(TKWT*SPSM) + I(TKWT*HT) + TSTWT + I(SPSM*KPS) +
                 I(KPS^2)+I(TSTWT^2)+SPSM+I(SPSM*KPSM)+I(TSTWT*HT)+  
                 I(TKWT*TSTWT)+TKWT+I(TSTWT*KPSM)+I(TKWT^2)+I(TKWT*KPS)+ 
                 I(TSTWT*SPSM)+I(TKWT*SPSM)+I(HT^2)+I(KPS*KPSM)+HT+           
                 barc67+I(KPSM*HT)+cmwg680bcd366+bcd141+barc86+     
                 gwm155+KPS+I(SPSM*HT), data = data2, x = TRUE, y = TRUE)
...
Output = matrix(NA, ncol = 6, nrow = length(AA)-1)
colnames(Output) = c('h', 'ERM1', 'ERM2', 'BIC', 'AIC', 'C')

for(l in 2:ncol(X_data)){
  dat = as.data.frame(X_data[, 1:l], IBLK = as.factor(data2$IBLK))
  Model1 = lm(YIELD ~ ., data = dat, x= TRUE, y = TRUE)
  X1 = Model1$x[,-c(1)]
  YIELD = Model1$y
  X = cbind(YIELD, X1)
  ...
  cat('The estimate vcdim is: ', range2[which.min(MerlinMat1)], " for Multi-Loc ", "\n")
  cat('The estimate vcdim is: ', range2[which.min(MerlinMat1r)], " for Multi-Loc ",  "\n")
  ...
  Output[l-1,] = c(range2[which.min(MerlinMat1)], round(ERM1), round(ERM2), round(BIC), round(AIC), c111
  )
  #dataa
...
apply(data.frame(Output[,-6], cv), MARGIN = 2, FUN = which.min)
which.min(abs(Output[,1]- seq(1,32)))

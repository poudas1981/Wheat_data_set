...
######################################################################################################
# Order of inclusion of covariates using SNP data in Licoln 99, this analysis include the design.
######################################################################################################

AA = Name[order(cor(ddd)[1,][-1], decreasing = TRUE)]
Big_order = lm(YIELD ~ I(TKWT*KPSM)+I(SPSM*KPS)+KPSM+I(TSTWT*KPSM)+I(KPSM*HT)+I(KPSM^2)+    
                 I(SPSM*KPSM)+I(KPS*KPSM)+I(TKWT*SPSM)+SPSM+I(SPSM*HT)+I(TSTWT*SPSM)+
                 I(SPSM^2)+I(KPS*HT)+I(TSTWT*KPS)+KPS+I(KPS^2)+I(TKWT*KPS)+ 
                 HT+I(HT^2)+I(TSTWT*HT)+barc12+barc67+TSTWT+        
                 I(TSTWT^2)+barc86+I(TKWT*HT)+gwm155+bcd141+cmwg680bcd366+
                 I(TKWT*TSTWT)+I(TKWT^2)+TKWT,
...
for(l in 2:ncol(X_data)){
  dat = X_data[, 1:l]
  est = Chxi(data = dat, NL = NL, B=5, m=10)

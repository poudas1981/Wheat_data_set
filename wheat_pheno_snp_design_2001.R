  ...
  Res = tryCatch(
    {
      ...
      FirstHalfN = matrix(NA, nrow =nrow(G1), ncol =m)
      FirstHalfQ = matrix(NA, nrow =nrow(G1), ncol =m)
      ...
      Para1 = matrix(Model1$coefficients, ncol = 1)
      #Para1 = (Para1[2:length(Para1)])
      ...
      Pred1 = (X2%*%Para1 - Y2)^2
      #tryCatch(Pred1 = (X2%*%Para1 - Y2)^2, error=function(e) NULL)
      #Pred1 = (predict(Model1 ,as.data.frame(X2))-Y2)^2
      for(ind in 0:(m-1)){
      ...
      SecondHalfN = matrix(NA, nrow =nrow(G2), ncol =m)
      SecondHalfQ = matrix(NA, nrow =nrow(G2), ncol =m)
      ...
      diff = abs(nustar2 - nustar1)}, error=function(e) NULL
  )
  Res
}
Chxi = function(data2, NL, B, m){
  ...
  for(i in 1:length(NL)){
    ...
    while(bprim < B+1){
      ...
      for(b in 1:B){
        set.seed(b*i*bprim)
        G1 <- data.frame(Date=as.Date(character()),
                         File=character(), 
                         User=numeric(), 
                         stringsAsFactors=FALSE)
        G2 <- data.frame(Date=as.Date(character()),
                         File=character(), 
                         User=numeric(), 
                         stringsAsFactors=FALSE)
        
        lev = levels(as.factor(data2$IBLK))
        for(j in 1:length(lev)){
          le1 = subset(data2, data2$IBLK == lev[j])
          for(k in 1:trunc(n/length(lev))){
            Ind = sample(nrow(le1),size = 2,replace = TRUE)
            Myle1 = le1[Ind,]
            # Step two: split the data into two groups
            Index = sample(nrow(Myle1),size = 2,replace = TRUE)
            G11 = Myle1[Index[1],]
            G22 = Myle1[Index[2],]
            G1 = rbind(G1,G11)
            G2 = rbind(G2,G22)
          }
          #cat("This is the ", j, " level","\n")
          
        }
        #cat("the dim of G1 is ", dim(G1), "\n")
        
        output = TryCatchLoop(G1, G2,m)
        if(is.numeric(output) == TRUE){
          cat('There is an estimate of chixi', '\n')
          Matxi[b,] = output
          
        }else{
          Matxi[b,] = rep('NA',m)
        }
        cat('The first bootstrap is ', b, 'the second bootstrao is, ', bprim, '\n') 
      }
...
for(l in 2:ncol(X_data)){
  dat = X_data[, 1:l]
  X = data.frame(cbind(scale(dat, center = TRUE, scale = TRUE),IBLK = as.factor(data2$IBLK)))

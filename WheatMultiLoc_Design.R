...
Func_Stat = function(df, data){
  data1 = data[,1:df]
  Model1 = lm(YIELD ~., data = data1)
...
BigModel = lm(YIELD ~ TKWT + TSTWT + SPSM + KPS + KPSM + HT + 
                ...
                I(KPSM^2) + I(KPSM*HT) +
                I(HT^2), data = data2, x=TRUE, y=TRUE)
Xdat = BigModel$x[,-1]
Ydat = BigModel$y
cor(Ydat, Xdat)
...
######################################################################################################
# Order of inclusion of covariates Multi-location analysis
######################################################################################################


AA = Name[order(cor(ddd)[1,][-1], decreasing = TRUE)]
Big_order = lm(YIELD ~ I(TKWT*KPSM) + I(TSTWT*KPSM) + KPSM + I(SPSM*KPS) + I(KPSM^2) + I(KPSM*HT) +
                 I(TKWT*SPSM) + I(TSTWT^2) + TSTWT + I(SPSM*KPSM) + I(KPS*KPSM) + I(TSTWT*SPSM) +
                 SPSM + I(SPSM*HT) + I(SPSM^2) + I(TKWT*TSTWT) + TKWT + I(TSTWT*HT) +
                 I(TKWT^2) + I(TKWT*KPS) + I(TSTWT*KPS) + I(TKWT*HT) + I(KPS*HT) + HT +
                 I(HT^2) + KPS + I(KPS^2),
...
for(l in 2:ncol(X_data)){
  ...
  for (j in 1:length(c1)) {
    for (g in 1:length(c2)) {
      ourestMat[j,g] = C1C2(est,ncol(data2)-1, NL, c1 = c1[j], c2 = c2[g])
      ourestMat2[j,g] = C1C2ratio(est,ncol(data2)-1, NL, c1 = c1[j], c2 = c2[g])
    }
  }
  ...
  ERM1 = Gaby(Loss = Risk2,eta = 0.05,
              n = nrow(data2), h = range2[which.min(MerlinMat1)], m=10)
  round(ERM1)

...
var = c('YIELD','HT', 'TSTWT', 'TKWT', 'SPSM', 'KPS', 'KPSM', 'IBLK')
data2 = data237[,var]
...
Output = matrix(NA, ncol = 5, nrow = length(AA)+1)
colnames(Output) = c('h', 'ERM1', 'ERM2', 'AIC', 'BIC')

for(l in 2:ncol(X_data)){
  dat = X_data[, 1:l]
  dat2 = cbind(dat, as.factor(data2$IBLK))
  Model1 = lm(YIELD ~ ., data = dat2, x= TRUE, y = TRUE)
  ...
  c11r = c1[Indexx2[1,1]]
  c22r = c2[Indexx2[1,2]]
  cat("C1 is ", c111, "c2 is ", c222, 'the number of col in data is  is', ncol(data2)-1, "\n")
  ...
  Output[l-1,] = c(range2[which.min(MerlinMat1)], round(ERM1), round(ERM2), round(AIC), round(BIC)
  )
  
}

cv_tmp = Model_cv(X_data, n_folds <- 10)

cv = colMeans(cv_tmp)
data.frame(Output[1:27,], cv)
Output
X_big = Big_order$x[,-1]
Y_big = Big_order$y
